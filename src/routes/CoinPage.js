          ...
          <div className="flex justify-between py-4">
            <div>
              ...
              {coin.market_data?.market_cap ? (
                <p>${coin.market_data.market_cap.cad.toLocaleString()}</p>
              ) : null}
            </div>
            <div>
              <p className="text-gray-500 text-sm">Volume (24h)</p>
              {coin.market_data?.total_volume ? (
                <p>${coin.market_data.total_volume.cad.toLocaleString()}</p>
              ) : null}
            </div>
          </div>
          ...
          <div className="flex justify-between py-4">
            <div>
              ...
              {coin.market_data?.high_24h ? (
                <p>${coin.market_data.high_24h.cad.toLocaleString()}</p>
              ) : null}
            </div>
            <div>
              <p className="text-gray-500 text-sm">24h Low</p>
              {coin.market_data?.low_24h ? (
                <p>${coin.market_data.low_24h.cad.toLocaleString()}</p>
              ) : null}
            </div>
          </div>
        </div>
